...
% Changed int32 to single 8/3/2020    
...
tt = (0:hours_per_step:(ndays*24-1))'; % units are hours since start_time
...
% ncwriteatt(outname,...
%     'time','units',['hours since ', num2str(start_year), '-01-01 00:00:00']);
ncwriteatt(outname,...
...
% Not specifying the FillValue because it causes an error
% Instead, using the default FillValue for the variable type
% ncwriteatt(outname,...
%     'mask','_FillValue', 0);
ncwriteatt(outname,...
...
ncwriteatt(outname, '/', 'creation_date', datestr(now));
ncwriteatt(outname, '/', 'institution', 'UCLA');
ncwriteatt(outname, '/', 'username', 'jschap');
ncwriteatt(outname, '/', 'version', 'VIC 5 image');
